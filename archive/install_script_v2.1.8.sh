...
init_var() {
  ...
  sql_215="alter table account change validity_period preset_expire int unsigned default 0 not null comment 'Default expiration time';alter table account add preset_quota bigint default 0 not null comment 'default quota' after preset_expire;update account set preset_quota = quota where last_login_time = 0;update account set quota = 0 where last_login_time = 0;alter table node add priority int default 100 not null comment 'priority' after port;INSERT INTO casbin_rule (p_type, v0, v1, v2, v3, v4, v5) VALUES ('p', 'sysadmin', '/api/account/clashSubscribeForSb', 'GET', 'default', 'default', 'default');"
}
...
install_mariadb() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-mariadb$") ]]; then
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-mariadb$" -f "status=running") ]]; then
      ...
      if [[ "${mariadb_user}" != "root" ]]; then
        echo_content yellow "---> MariaDB ${mariadb_user}The database password (please keep it safe): ${mariadb_pas}"
      fi
    else
...
install_redis() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-redis$") ]]; then
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-redis$" -f "status=running") ]]; then
      ...
      echo_content yellow "---> RedisThe database password (please keep it safe): ${redis_pass}"
    else
...
EOF
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-ui$" -f "status=running") ]]; then
      ...
      echo_content red "---> Trojan Panel Frontend Installation failed or abnormal operation, please try to repair or uninstall and reinstall"
      exit 0
    fi
  else
...
install_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel$" -f "status=running") ]]; then
      ...
      echo_content red "---> Trojan Panel Backend Installation failed or abnormal operation, please try to repair or uninstall and reinstall"
      exit 0
    fi
  else
...
install_trojan_panel_core() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-core$") ]]; then
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
      ...
      echo_content red "---> Trojan Panel Kernel Installation failed or abnormal operation, please try to repair or uninstall and reinstall"
      exit 0
    fi
  else
...
update_trojan_panel_ui() {
  ...
  echo_content yellow "Tip: The current version of Trojan Panel Frontend (trojan-panel-ui) is ${trojan_panel_ui_current_version} The latest version is ${trojan_panel_ui_latest_version}"
...
update_trojan_panel() {
  ...
  echo_content yellow "Tip: The current version of Trojan Panel Backend (trojan-panel) is ${trojan_panel_current_version} The latest version is ${trojan_panel_latest_version}"
...
update_trojan_panel_core() {
  ...
  echo_content yellow "Tip: The current version of Trojan Panel Kernel (trojan-panel-core) is ${trojan_panel_core_current_version} The latest version is ${trojan_panel_core_latest_version}"
  ...
  if [[ "${trojan_panel_core_current_version}" != "${trojan_panel_core_latest_version}" ]]; then
    ...
    if [[ -n $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
      echo_content skyBlue "---> Trojan Panel Kernel update completed"
    else
...
uninstall_all() {
  ...
  echo_content skyBlue "---> Uninstall all Trojan Panel related containers完成"
}
  ...
  if [[ ! $(docker -v 2>/dev/null) ]]; then
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-caddy$") ]]; then
      if [[ -z $(docker ps -q -f "name=^trojan-panel-caddy$" -f "status=running") ]]; then
        echo_content red "---> Caddy2 runs abnormally and the running log is as follows："
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-redis$") && -z $(docker ps -q -f "name=^trojan-panel-redis$" -f "status=running") ]]; then
      echo_content red "---> Redis is running abnormally logs are as follows："
    ...
    if [[ -n $(docker ps -a -q -f "name=^trojan-panel-core$") && -z $(docker ps -q -f "name=^trojan-panel-core$" -f "status=running") ]]; then
      echo_content red "---> Trojan Panel Kernel runs abnormally, the log is as follows："
